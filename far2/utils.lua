...
local function MakeAutoInstall (Plugin, AddUserFile)

  local ModuleDir = Plugin.ModuleDir or PluginDir
  ...
  local function AutoInstall (startpath, filepattern, depth)
    ...
    ---------------------------------------------------------------------------
    local startpath = ModuleDir .. startpath:gsub("[\\/]*$", "\\", 1)
    local filepattern = filepattern or "^_usermenu%.lua$"
    ---------------------------------------------------------------------------
    local first = depth
    local offset = ModuleDir:len() + 1
...
local function LoadUserMenu (Plugin, aFileName)
  --far.Show("LoadUserMenu", aFileName)
  ...
  ------------------------------------------------------------------------------
  env.AutoInstall = MakeAutoInstall(Plugin, env.AddUserFile)
